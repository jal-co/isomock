...
export function zoomFramingAround(
  ...
  const zoom = Math.min(ISOMOCK_ZOOM_RANGE.max, Math.max(ISOMOCK_ZOOM_RANGE.min, nextZoom));
...
export function panFraming(
  ...
  const frameScale = getIsomockFrameScale(framing.zoom, imageAspect, frameAspect);
  return {
    offset: { x: framing.offset.x + delta.x / frameScale, y: framing.offset.y + delta.y / frameScale },
    zoom: framing.zoom,
  };
}
...
export function useIsomockFramingGestures(
  ...
  const gesture = React.useRef<{ framing: Framing; group: string; lastAt: number; pinchStartZoom: number } | null>(null);
  ...
  React.useEffect(() => {
    ...
    const begin = (): { framing: Framing; group: string; pinchStartZoom: number } => {
    ...
    const locate = (event: Event & { clientX?: number; clientY?: number }) => {
    ...
    const wheel = (event: WheelEvent) => {
      ...
      const unit = event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? located.frameHeight : 1;
      if (event.ctrlKey || event.metaKey) {
        commit(zoomFramingAround(start, start.zoom * Math.exp(-event.deltaY * unit * 0.01), located.anchor, located.imageAspect, located.frameAspect));
      } else {
        commit(panFraming(start, { x: (-event.deltaX * unit) / located.frameHeight, y: (-event.deltaY * unit) / located.frameHeight }, located.imageAspect, located.frameAspect));
      }
    };

    const pinch = (event: Event) => {
      const located = locate(event as Event & { clientX?: number; clientY?: number });
      if (!located) return;
      event.preventDefault();
      event.stopPropagation();
      const current = begin();
      const scaleFactor = (event as Event & { scale?: number }).scale ?? 1;
      commit(zoomFramingAround(current.framing, current.pinchStartZoom * scaleFactor, located.anchor, located.imageAspect, located.frameAspect));
    };
